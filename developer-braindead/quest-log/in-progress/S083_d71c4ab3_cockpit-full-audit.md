...
**Cascade.** Dev-brain: this quest log, `respawn.md`, `comms/active.md`, `bank/build-lessons.md` (4 S083 lessons incl. the regression), `spellbook/session-close.md` (harvest-learnings step). Cross-project memory: `feedback-instrument-dont-reguess`, `feedback-autonomous-verify-what-you-can`, `feedback-documented-tradeoff-isnt-acceptance`. **Main-brain changes (`gielinor/`):** `spellbook/rituals/close-session.md` — 5th harvest question (corrections/reverts → drafts + memory), principal-directed.
**Cockpit commits:** e04c679 terminal re-pin/hardening · f5222d1 backend crash-guard · f8d54c0 dead --closing var · dc6b0ca regression test · 317ac42 busy→idle decay (**reverted**) · 04848db Shift+Enter preventDefault+diag · 642db9c instant Esc-clear · f161b57 REVERT busy-decay (Jebrim false-idle) · 916e6f8 regression lesson · f63a050 ritual harvest-learnings. Plus dev-bookkeeping commits.

- L7 (round 4): **REGRESSION caught by principal** — my 90s busy→idle timeout false-tripped a live Jebrim session (showed IDLE while working). Reverted [f161b57]; the Esc-keystroke detection stands alone as the cancel fix; regression-guard test added. Harvested the lesson (documented-tradeoff-isn't-acceptance) → build-lessons + memory. Then per principal directive, made correction/regression harvesting a REQUIRED step in both session-close rituals [f63a050]. Lesson: a self-named failure mode is a blocker until evidenced rare; prefer a ground-truth signal over a silence-heuristic; documentation ≠ judgment.
