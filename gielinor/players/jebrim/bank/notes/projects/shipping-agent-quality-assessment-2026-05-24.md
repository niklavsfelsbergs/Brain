...
**As-of:** 2026-05-24 ([[S059_9369b3f2_shipping-agent-limit-testing|S059]] testing campaign — docs-only audit + 5 single-shot calls + 3 multi-round investigations against the live mart). Re-verify the improvement-status lines before relying; none were implemented this session (principal chose log-first).

Durable read on how the shipping-agent (`Documents/GitHub/shipping-agent/`) actually performs against its own rulebook, distilled from [[S059_9369b3f2_shipping-agent-limit-testing|S059]]. Source narrative: `quest-log/in-progress/S059_9369b3f2_shipping-agent-limit-testing.md`.

## Headline

The rulebook is **not decorative — it's followed and it materially changes outputs.** The `savings-investigation` falsification gate (added [[S059_9369b3f2_shipping-agent-limit-testing|S059]]) fired correctly in 3 separate tests; cost-basis discipline, coverage decomposition, undefined-column refusal, and scope-decline all held on live questions. The agent never broke; pushed twice on its weakest point, it conceded and corrected each time.
...
## Improvement backlog — IMPLEMENTED + committed [[S059_9369b3f2_shipping-agent-limit-testing|S059]] (principal: "do it all")
...
## Uncommitted in the shipping-agent repo (as of [[S059_9369b3f2_shipping-agent-limit-testing|S059]])
...
## [[S060_7cd31d19_shipping-agent-training-campaign|S060]] follow-up (2026-05-24) — re-test + teachings applied
...
- **[[S059_9369b3f2_shipping-agent-limit-testing|S059]]'s headline fix LANDED for cause-attribution.** Rate-vs-mix decomposition + own-counter falsification now self-trigger first-pass, unprompted (H2: refused a false "UPS +20% in April" premise, decomposed it to fuel + oversize-mix, gated its *own* counter-claim without challenge). The asymmetric-skepticism gap [[S059_9369b3f2_shipping-agent-limit-testing|S059]] named is closed at the cause layer.
...
**Same known root, fourth location.** "Full rigor present, doesn't self-trigger on the fast path" — cause-attribution ([[S059_9369b3f2_shipping-agent-limit-testing|S059]], fixed) → scope/denominator ([[S060_7cd31d19_shipping-agent-training-campaign|S060]]) → metric-set coherence (board-numbers) → **systemic-vs-local null / transient state** (here). The generative fix family holds: a fast-path self-gate, not another scar.
...
Third board-prep transcript, full convo this time. Prompt chain: April-2026 YoY per-parcel (+17%) → push to decompose → counterfactual ("hold 2025 oversize + fuel") → "final consensus" → "investigate FedEx." Harvested in [[S067_6ccc2220_shipping-agent-bucket-first-harvest|S067]].
...
**Same root family, fifth location.** "Full rigor present, doesn't self-trigger on the fast path": cause-attribution ([[S059_9369b3f2_shipping-agent-limit-testing|S059]], fixed) → scope/denominator ([[S060_7cd31d19_shipping-agent-training-campaign|S060]]) → metric-set coherence (board-numbers) → systemic-vs-local null ([[S064_1fc49f17_shipping-agent-orwo-quota-reload-rule|S064]]) → tool-sequencing/bucket-first ([[S067_6ccc2220_shipping-agent-bucket-first-harvest|S067]]) → **dimension-selection / which-cut (here)**. Sibling of [[S067_6ccc2220_shipping-agent-bucket-first-harvest|S067]] (which *tool* to reach for first) and the `savings-investigation` falsification gate (turn the gate on your own answer): here the gate is *"what cut would make this NOT a mistake?"*
...
**Removed from the menu per principal:** `current_shipping_status` (quality too low to cut on), `is_returned` (NOT approved — stays do-not-use; **this resolves the [[S068_363fdec7_v1-freeze-mart-knowledge-reconciliation|S068]] A5 ruling**). Also flagged unusable: `shipping_region` (64,177 dirty free-text → use `destination_country` + zip-prefix), `packagetype_group` (empty, no source in V1).

**Status (2026-05-25):** `tables.md` "Dimensions you can slice by" catalogue **DRAFTED in-file** (low-risk, lands sooner). `how_to.md` rule **HELD** for post-demo (2026-05-26) — demo-stability + how_to.md is actively dirty under a parallel session ([[S070_a43c1779_shipping-agent-mode2-trigger-and-menu-rendering|S070]] Mode-2 work). Land the rule after the demo, on top of whatever [[S070_a43c1779_shipping-agent-mode2-trigger-and-menu-rendering|S070]] commits.

## 2026-05-25 — Mode 2 trigger sharpening + interactive-menu rendering ([[S070_a43c1779_shipping-agent-mode2-trigger-and-menu-rendering|S070]])
...
## 2026-05-25 — [[S071_35eca9a3_shipping-agent-drift-test-10q|S071]] 10-Q drift test (post-hardening regression; the root did NOT reproduce)

Principal-cued 10-question test (7 recent-change probes + 3 core), embodied agent in 10 read-only dwarves vs the LIVE mart, graded vs the working-copy rulebook (how_to.md dirty = [[S070_a43c1779_shipping-agent-mode2-trigger-and-menu-rendering|S070]] menu edits in tree, on @cd0b0e3). **Every number ground-truth-verified; zero hallucination across all 10.** Source narrative: `quest-log/in-progress/S071_35eca9a3_shipping-agent-drift-test-10q.md`.
...
- **Test-method limits:** r36 reload-branch untestable (cost fully repopulated post-reload — invoice 12.26M / null ~5%); interactive-menu *blocking* only simulated (dwarves can't truly wait); single-shot per Q, so multi-turn basis-switch ([[S067_6ccc2220_shipping-agent-bucket-first-harvest|S067]] M2/M3) not re-probed this round.
...
**Pattern shift worth carrying:** four prior campaigns ([[S059_9369b3f2_shipping-agent-limit-testing|S059]]→[[S070_a43c1779_shipping-agent-mode2-trigger-and-menu-rendering|S070]]) each found a NEW location of the self-trigger gap and patched it. [[S071_35eca9a3_shipping-agent-drift-test-10q|S071]] is the first where the *whole family* held under a fresh battery. The generative-self-gate thesis is earning out — the remaining work is doc-hygiene (D1) and proving the *newest* patch (Root E / dimension-scan) is generative not scar-matched (D2), not finding a 7th location. NO shipping-agent edits this session (held WIP, pre-2026-05-26 demo).

## 2026-05-25 — [[S072_35eca9a3_shipping-agent-audit-3|S072]] pre-demo audit-3 + fixes shipped + re-test (D2 proven generative)

Second full audit (after [[S054_50b00902_shipping-agent-audit-2|S054]] audit-2); principal said "fix it all" → implemented + re-tested, all pre-demo (2026-05-26). 4 read-only auditor dwarves (how_to / reference / skills / cross-cutting+harness) → ~18 findings; fixed in 4 commits on picanova-tracked main (e6be3da→f02c6e0), then a 6-probe embodied re-test — **all PASS, zero regressions, every number ground-truthed.** Source: `quest-log/in-progress/S072_35eca9a3_shipping-agent-audit-3.md`.
...
**Col count 65 CONFIRMED** (information_schema) — resolves the open H3 (65-vs-63) from the [[S071_35eca9a3_shipping-agent-drift-test-10q|S071]] section; docs are correct.
