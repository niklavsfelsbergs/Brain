...
---
## Continuation — session 4c2210ee (2026-05-28)
Picked up the report-hardening item the principal had OFFERED but not green-lit; principal said go.
**Done (report.py prose hardening, decision_report.html regenerated + verified):**
- Trustworthy-vs-provisional split: added `HELD_ENGINES={fedex,dhl_paket,guell}` + `_is_trustworthy()` classifier; KPI strip now leads with trustworthy full-coverage leader `renew_maersk_plus_hermes` €247,716 (8.5% of €2.91M) and a separately-labelled "provisional ceiling" `renew_maersk_plus_fedex` €379,052; per-row TW/PROV badge added to the decision-set table (17 TW / 73 PROV of 90).
- Decision-so-far folded into the summary: renew Maersk+Hermes €248k full cover (leader, Maersk-alone strands ~34k parcels); GLS alternate `renew_maersk_drop_dpd_pl_plus_gls_dhl_express` €196k full cover; `add_hermes` €224k. Why raw top isn't the answer (FedEx/Güll held engines under-price).
- Two material caveats callout: dpd_pl CH customs €484k + gls EFTA €278.9k, both collapse under consolidated customs (CCD).
- Full Section-04 carrier-narrative rewrite (principal chose full rewrite) on current scenarios/cost_matrix numbers; bias ratios re-read (Maersk 0.571, Hermes 0.858, DHL Express 1.290, GLS 1.059, DPD-PL 1.077; held FedEx 0.885/Güll 0.775/DHL Paket 0.997); added FedEx narrative (was missing). Refreshed Section-01 frontier callout, the proxy-fuel methodology callout (now "confirmed-on-rebuilt-engines + band"), the what-changes DPD-PL row, docstring.
- Verified: clean regen, all KPI/badge/caveat elements present, numbers ground-truthed. NOT visually pixel-eyeballed (no GUI here) — opened in browser for principal.
**NOT committed** (principal-gated). Next: full-year scoping feasibility check (parked decision basis).

## Full-year scoping feasibility (session 4c2210ee, 2026-05-28)
Verdict: **2025 full-year population pull is FEASIBLE (green) with a source repoint.**
- Current `sql/population.sql` reads `enterprise_silver.fact_shipments` + `enterprise_silver.fact_shipment_cost_summary`. Live probe (Redshift MCP, tcg_nfe): **silver fact_shipments still exists, but silver fact_shipment_cost_summary is GONE** (gold cutover). So the current pull's cost-summary join would BREAK on re-pull → repoint BOTH to `shipping_mart.*` (gold).
- Gold parity confirmed: all 22 fact columns the pull selects exist on `shipping_mart.fact_shipments`; all 14 cost buckets exist on `shipping_mart.fact_shipment_cost_summary` (+ _local variants). Clean 1:1 repoint, no schema gymnastics. (Keepsake "silver gone / 11-bucket" was partially stale — verified live.)
- Volume: 2025 full-year in-scope (PCS PL, 18 countries, dims/weight/packagetype non-null) = **2,926,069 shipments** (vs ~528k Q1 2026). 2024 = 3.18M also available. Data floor covers it.
- Compute: full-year cost_matrix ≈ 2.93M × 9 engines ≈ **26M rows** (~5.5× the 4.76M Q1 matrix) → chunk the build by month/carrier given OOM history ([[S069_006248ef_pipeline-oom-hardening|S069]]/[[S097_1ce9fc1f_scm-daily-product-temp-spill-oom|S097]]). Not a blocker.
- Modelling subtlety: engines are per-shipment/stateless so the population "replays identically"; product/Q4-mix shift is captured automatically via real 2025 shipment weights/dims (no product column needed). The real_* 2025 buckets are NOT the full-year baseline (they're 2025 prices) — the do-nothing baseline must re-price the 2025 population on incumbents' 2026 rates. The load-bearing new work is wiring the seasonal peak/demand surcharges (~zero in Q1) + the baseline re-price, not the pull.
- Basis recommendation: Option 1 (replay 2025 actuals on 2026 cards) — most faithful, feasibility confirmed. Awaiting principal's basis pick before writing DECISIONS.md.

**Basis decision (principal):** Option 1 (replay 2025 actuals on 2026 cards). Recorded in tender docs: DECISIONS.md (new top entry, basis + feasibility), NEXT.md (item 3 = 5-step Opt-1 build plan + "last updated"/GOAL-REFRAME lines), FULL_YEAR_SCOPING_NOTE.md (PARKED→BASIS CHOSEN). Build NOT started (next phase). All tender + brain changes UNCOMMITTED — awaiting principal go.
