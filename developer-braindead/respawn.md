...
**Last updated.** 2026-05-23 (end of [[S047]] — visualizer: cluster-stack + helmet clearance. `relayoutBubbles()` rewritten as parent-based grouping — sub-agents stack vertically anchored to principal sprite X, principal at the bottom keeps the trail, non-bottom members suppress their trails. Pass 4.5 keeps the stack rigid under inter-group collision push. `GATHER_SLOTS.bubbleY` lifted -40 across the board (S039 left bubbles "sitting on the helmet"; now ~65 px clearance). No-slot fallback `-68 → -108` in renderIntent + cluster `baseYOff` default. **One real surface carried forward, high-priority:** parallel-Braindead spawn failure — my own WAITING session had no sprite while the IDLE Braindead·3 sibling did; pulsating speech-cloud is the orphan-clear loop (intent set → no sprite → reconcileFromManifest Pass 4 clears → next intent re-creates). Same root family as D-025 #1 but specific to parallel-instance Braindead. Full carry in `quest-log/in-progress/S047_a110d573_visualizer-cluster-stack-and-helmet-clearance.md`. Prior: [[S044]] D-024 scaffold + comms chatbox.).
