...
- **c03f33b** — `chat.ndjson` as a hook-side humanized stream. `emit-event.py` now appends one human-language line per event (`Braindead is editing index.html`, `Jebrim ran grep over sql/`, `Guthix is reading bank/notes/`, idle/spawn/despawn). The chat panel reads this directly — no more deriving prose from raw event verbs client-side. Switchboard row subtitle (the `latest_action` slot from [[S049_17e701eb_visualizer_state_aware_motion_and_action_line|S049]]) repurposed as a one-line "what is this session doing right now."
