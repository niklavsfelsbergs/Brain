...
- **"Don't pre-build for hypothetical scale" promoted from S030 carry into a new spec.** The `_about.md` explicitly says *"If the folder grows to the point that browsing becomes painful, a triage ritual can be added then — not now."* Same posture S030 used for the research/ folder ("no draft gate inside the folder"). When a new folder lands, name what won't be built — saves a future audit pass over speculative scaffolding.
