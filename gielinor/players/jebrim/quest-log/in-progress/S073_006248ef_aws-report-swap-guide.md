...
- Effect: every scheduled 08:00 run = FULL Redshift pull (not 3-mo incremental). Works (S069 OOM fix in image, 20Gi pod) but heavy. WATCH DAG `execution_timeout=30min` on first scheduled run vs ~18M-row mart; timeout = no refresh that day (dashboard keeps last-synced data, not fatal).
