...
- §O.4 `[~]` **gielinor/ — DRY-RUN done ([[S099_acf8fc80_obsidian_quest_link_migration]]), apply DEFERRED.** Scope is small + clean: **42 rewrites / 17 files** (gielinor's own `D-017`…`D-026` + player quests `S058`/`S069`; `[[D-024_*]]` heaviest at 18 refs). The 3 flagged dupes (S014/S062/S076 — per-player `SNNN` collisions) all have **0 inbound links** → no decision needed. **Apply blocked this session: Guthix was live in gielinor** (meta/lorebook/players/rituals are his bankstanding surface — D-024 race). **To apply next:** (a) confirm no live jebrim/zezima/guthix, (b) extend the script's `CONVENTION_DOCS` set to gielinor's convention docs (`meta/`, the `_about.md` files), (c) diff-review, (d) `--apply`. Reuse `bank/research/obsidian-link-migrate.py --vault <gielinor>`.
