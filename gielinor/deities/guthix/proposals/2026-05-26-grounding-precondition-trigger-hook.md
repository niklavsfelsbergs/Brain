...
**Drafted:** 2026-05-26 (bankstanding [[B-008_2026-05-26_eighth-bankstanding|B-008]]). **Status:** active draft for principal review.

## 1. Observation

The grounding-precondition (read active context + confirm referent identity before substantive output) is confirmed globally (`examine/confirmed/.../grounding-precondition...`, "G1"), graduated at [[B-007_2026-05-25_seventh-bankstanding|B-007]] from two independent player instances, and stored in memory (`feedback_anchor_referent_before_analyzing`, `feedback_check_own_memory_before_working_repo`). It still failed three more times since:

- **[[S066_e2362ea0_domajamas-dalas-reconsider|S066]] (Zezima)** — analyzed a flat as "new" that was [[S056_e5df54a2_apartment-domajamas-dalas-research|S056]] apartment #1.
- **[[S076_949a59cf_scm-alert-engine-audit|S076]] (Jebrim)** — grepped only the working repo for "the message I prepared"; it was in his own `bank/notes/`.
- **[[S095_f60153e0_gertrudes113-buy-deliberation|S095]] (Zezima)** — re-derived the folio analysis cold before recognising it as Gate 2 of the [[S066_e2362ea0_domajamas-dalas-reconsider|S066]] deep-dive.
...
- **On match, inject `additionalContext`** (non-blocking) along the lines of:
  > *Grounding cue detected ("{matched}"). Before analyzing: search the active player's `bank/notes/`, `research/`, and `quest-log/` for this subject; map any uploaded artifact to where prior work left off; confirm the referent's identity. Treat "this is new" as a hypothesis to disprove. (G1; recurred [[S066_e2362ea0_domajamas-dalas-reconsider|S066]]/[[S076_949a59cf_scm-alert-engine-audit|S076]]/[[S095_f60153e0_gertrudes113-buy-deliberation|S095]].)*
...
- **A respawn-ritual first-message step** (scan the opening message for cues, do a self-bank check before accepting input). Complementary, but only covers session *start* — [[S076_949a59cf_scm-alert-engine-audit|S076]]'s miss was mid-session. The hook covers every turn. Could land both; the hook is the higher-coverage primary.
