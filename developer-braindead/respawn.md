...
**Step 0 — Parallel-Braindead spawn failure (S047 carry).** A WAITING Braindead session's intent narration keeps trying to render a bubble that gets immediately wiped by the orphan-clear pass — because no `actor-braindead` DOM node exists for the live instance even though the switchboard knows the session is alive. Visible as a pulsating speech cloud on the principal's screenshot. Full investigation surface + symptoms + the 4 hypotheses to chase in `quest-log/in-progress/S047_a110d573_visualizer-cluster-stack-and-helmet-clearance.md` § "Open — for next session." Start by reproducing with two live Braindead sessions, devtools open, breakpoint in `ensureActorExists` for `actor === 'braindead'`. Same root family as [[D-025]] #1 but the S042 `915ff92` fix didn't reach the parallel-instance case. **Block other Step 0/0a work until this lands** — every dev-brain session is currently miscoordinating its own sprite.
