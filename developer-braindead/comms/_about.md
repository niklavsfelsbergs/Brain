...
- **Body cap: 2–3 lines, ≤120 chars each.** This is a coordination channel surfaced in the visualizer's chat panel, not a session journal. Detail belongs in the quest-log; the comms entry references the SNNN. Born [[S043_switchboard_state_visibility|S043]] (2026-05-22) — the gielinor twin shipped under this rule from day one after the dev-brain shape produced multi-paragraph press-release bodies that were unreadable in the chat panel.
- **Conversational, in voice.** "Wrapping up [[S042_visualizer_character_audit|S042]] — shipped 4 of 6 audit fixes, #2 handed to @braindead-f9da453a" — not a structured `Completed:` / `Leaving open:` block with sub-points. The protocol *allows* the structured shape; the discipline says don't lean on it when one line will do.
