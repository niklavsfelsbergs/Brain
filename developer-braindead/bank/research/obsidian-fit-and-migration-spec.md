...
- **NEXT:** (1) **verify in Obsidian** — open `developer-braindead/` as a vault; the decision graph + backlinks should light up (principal eyeball; not headless-checkable). (2) extend to **dev-brain quests** (`SNNN_dN/_pN` groups → main entry; the 3 real dupes S038/S060/S086 need a human call) then **gielinor/**. (3) two fuzzy D-001 calls (`plan.md:29`, `S003:13`) left as-set — both resolve. (4) cross-brain ~4% refs decision. Roadmap: [[plan]] §O.
