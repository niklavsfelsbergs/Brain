...
**Last updated.** 2026-05-23 (end of [[S044]] — D-024 scaffold + comms-driven chatbox. Scaffolded `gielinor/comms/` (file + `_about.md`), updated `gielinor/spellbook/rituals/respawn.md` + `close-session.md` + `meta/layer-routing.md` per [[D-024]]. Wired visualizer COMMS panel to render both `comms/active.md` files via a sidecar mirror; suppressed intent + action echo to chat in LIVE mode so bubbles carry "what I'm doing" and chat carries "what we're saying to each other." Tightened comms body discipline in both `_about.md` files after the first live render shipped multi-paragraph press-release entries to the chat panel. Folded body[0] into the header so collapsed view is informative (▸ only when there's hidden detail). Prior: [[S043]] switchboard state visibility.).
