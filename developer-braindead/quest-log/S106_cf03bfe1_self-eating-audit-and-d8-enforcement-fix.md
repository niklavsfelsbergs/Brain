...
- **🔴 The real bug (D8):** 4 of 6 "architectural guarantees" (dwarf/gnome/penguin write-boundaries + no-sub-spawn) are **inert from brain root** — the cockpit's launch dir. They gate on `payload.agent_type ∈ {dwarf,gnome,penguin}`; no `dwarf` config exists, and `gnome`/`penguin` live in `gielinor/.claude/agents/` which isn't loadable at brain root. So real spawns are `general-purpose` → no match → exit 0. S085-class gap, uncaught ([[S085_5f93bb32_cockpit-pty-auth-and-md-xss|S085]] only re-tested the two path-based hooks).
- **🟠 Inward/outward (D2):** ~65% inward; the single most expensive build (cockpit/visualizer line) exists to *watch the agent work*. [[D-027_inward_outward_build_imbalance|D-027]] borne out and live.
...
3. **The audit's two real problems** beyond D8: build the outward half (§C / [[D-027_inward_outward_build_imbalance|D-027]]); land the deferred OPEN hook (D3).
