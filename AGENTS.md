...
**The entry sequence includes sibling-detection and posting an `OPEN` to `developer-braindead/comms/active.md`** (`respawn-ritual.md` steps 6–8). A mid-conversation pivot via this cue is **not** exempt — it runs the same ritual a clean respawn does. This is the system's most common discipline leak: the recurring *"did not post an OPEN — dev-brain entered mid-conversation"* note across `comms/active.md` meant ~70% of sessions skipped it historically. **Current measured state (2026-06-09 S177 health-check): first-try OPEN-posting is ~72% — not the ~100% earlier audits claimed — with the `require-open` write gate backstopping the rest, so never-posted is ~0. The gate is a backstop, not the procedure: post the OPEN first-try, before the gate has to remind you.** `OPEN` is the half that *prevents* collisions (announce your files, check for live siblings before editing); `CLOSING` only records them after. Fire it on every entry.
