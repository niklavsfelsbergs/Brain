# SCM alerts — ORWO/TCG entity split (S098)

**As of:** 2026-05-26 (S098). Shipped: bi-analytics `54cde63` → merged to main `3535229`. App: `NFE/dashboards/shipping_costs_monitoring_nextjs/`.
...
The detection engine (just stabilized in S055+S076) is treated as a **black box run once per entity**, rather than re-keying its ~20 internal corridor group-bys:
...
- `entity` stamped onto `alerts.parquet` / `issues.parquet`. Deviations stay **global** (Deviations tab only; deviation_blowout was demoted out of alerts in S076).
