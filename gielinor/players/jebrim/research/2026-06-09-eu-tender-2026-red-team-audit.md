...
- **Status: UNCONFIRMED — it may be correct scoping.** I cannot tell from the committed repo whether the 3-carrier / FR-only baseline is a deliberate EU-scope filter or an accidental subset. That is exactly why it's the first thing to confirm.
...
1. **Confirm the MAERSKUK / baseline-scope question (A0).** Is the 3-carrier, FR-only Q1 baseline a deliberate EU-scope filter? If MAERSKUK is in scope, the Maersk lever is mis-sized. *Highest priority — it questions a foundation.*
