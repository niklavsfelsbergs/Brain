...
**Rule.** When harvesting learnings from an agent conversation: (a) route each learning to **both** my brain *and* the agent's own rulebook where it made the error — the brain teaches me, only the rulebook changes the agent; and (b) re-read the **full** conversation for every correction/pushback beat before declaring the harvest complete. The headline lesson is rarely the only one — each place the principal pushed is a candidate rule.

**Related:** the 5 learnings this produced — skill [[dimension-gate-carrier-swap-savings]], skill [[savings-investigation-deliverable-shape]], examine [[2026-05-23-inherited-confidence-not-own-confidence]], examine [[2026-05-23-money-figures-state-their-period]].
