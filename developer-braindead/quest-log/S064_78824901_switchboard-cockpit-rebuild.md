...
5. **Coordination:** found a live sibling **braindead-3d2dc4b1 ([[S063_switchboard_feed_liveness_and_cache_fix|S063]])** on the incremental-patch path (cache-bust, rename COMMS). Surfaced it; principal chose **greenfield + keep old board live**. Flagged the sibling off the doomed polish in comms.
...
- `backend.py` grew the driver, lifted near-verbatim from the proven `switchboard/server.py` ([[S060_brain_self_audit_and_plan_reconciliation|S060]]) minus the unused `/pty`: `/chat` WS (headless `claude` stream-json; input + `{type:interrupt}`→control_request; `?resume=<uuid>`), `/history?session=<uuid|sid8>` (`.jsonl`→visual turns via `parse_transcript`), all `no-store`.
