...
**Date observed:** 2026-05-22 (S034 — EU tender logic review)
**Anchor:** S034 D1 (Maersk audit dwarf) vs S034 F1 (Maersk fix dwarf) on the IT/ES oversize trigger.

## The observation

In S034 I spawned 7 audit dwarves (D1-D7) in parallel to find calc bugs + wrong/missing assumptions across the EU tender Phase 2 codebase. D1 reported (high severity) that the Maersk IT oversize encoding was wrong — `max_height=170` from "Height 170-240 cm" was compared against `d_min` (unreachable), making the height check vestigial.
...
4. **When the audit-then-fix loop fires in the same session (as in S034), trust the fix dwarf's framing over the audit dwarf's** when they disagree. The fix dwarf had to actually solve the problem.
