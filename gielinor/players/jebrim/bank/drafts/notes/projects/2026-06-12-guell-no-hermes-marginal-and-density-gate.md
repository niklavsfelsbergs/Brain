...
## BUILT ([[S230_b94d4675_eu-tender-no-hermes-with-gull-report|S230]], 2026-06-12) — the report's q1-basis number

Report built: `bi-analytics-main/.../2_analysis/final_report_no_hermes_with_gull/` (six-carrier, beside `final_report_no_hermes_v2/`). Headline **€1,139,121/yr (7.55%)**; **Güll net marginal +€163,097/yr** over the €976,024 five-carrier floor.

**The "expect smaller" prediction was WRONG — it came out ~equal** (+€163,097 vs the decision-set's +€163,897, an €800 gap on €164k). The two pipelines disagree on UPS pricing (invoice vs engine) and DBS treatment, yet the Güll marginal landed within 0.5% — coincidence, not a structural identity; don't read it as "the pipelines agree." What DID differ structurally: the **per-country split inverted** — q1-basis routes **CH 44,270 / AT 42,966**, vs the decision-set's AT 79,497 / CH 31,523. UPS-on-engine + the q1 population reshape which lanes Güll wins. Gross offer saving €205,479; net marginal €163,097 (gap = volume cannibalized from Maersk/UPS/DPD). The DBS-freight pin neutralizes the decision-set "DBS sliver" artifact by construction (DBS-origin can't route to Güll). DEFENSIBLE floor still €60–120k — the density/parity caveats are unchanged by the build; the build only fixed the *paper* number in the report's basis. [[feedback_synthetic_pass_not_live_correct]] cousin: a predicted direction ("smaller") is a hypothesis until the pipeline actually runs.

## DENSITY GROUNDED ([[S233_14a9b5f3_guell-linehaul-density-correction|S233]], 2026-06-12) — the marginal collapses to ~€0

The 150 was the load-bearing lever, and it was ~2–3× too high. Two shipping-agent mart pulls ([[S231_shipagent_guell-pallet-fill-substitutes|S231]] / [[S232_shipagent_guell-loading-unit-by-destination|S232]]): the gold `shipping_mart` carries a **loading-unit** (pallet-grain) grouping ~96% populated on Szczecin (Güll's origin). Q1-2026 Szczecin loading units average **~52 parcels** (median 33, n=11,739 — robust); geometric volume ceiling ~80 (AT) / ~101 (CH); weight cap not reached until ~176/~223. So 150 sits above the AT volume ceiling — these flat parcels are **volume-bound, not weight-bound**. Single-dest cut ([[S232_shipagent_guell-loading-unit-by-destination|S232]]): CH-only ~70, AT-only ~20 (partials artifact — AT isn't dedicated today); both thin (249/139 units). Neither lane clusters near 150.

**Engine corrected (guell-2.0.0, uncommitted):** the flat `PARCELS_PER_PALLET = 150` split into `PARCELS_PER_PALLET_INBOUND = 52` / `_AT = 75` / `_CH = 75` — a single density across the mixed inbound sprinter and the dedicated outbound pallets was never lane-appropriate. Per-parcel line-haul: inbound **€0.80→€2.30** (the dominant move — €955 ÷ 8 pallets ÷ density), outbound AT €0.16→€0.33, CH €0.27→€0.53. 19/19 engine tests pass. Also fixed `build_stats_no_hermes_with_gull.py`, which hardcoded 150 into the JSON → now reads the split constants.

**Result:** regenerated cost matrix → stats → report. Güll's routed volume recomputed live: **~87k → 308 parcels/yr** (CH only); the per-cell routing sheds Güll once it's dearer than DHL/DPD/UPS/Maersk. **Net marginal +€163,097 → +€520/yr.** The €163k was almost entirely the density artifact. 5-carrier floor (€976,024) unchanged; reconciliation Δ€0. The earlier **"defensible €60–120k" floor was a pre-grounding guess — obsolete**; the grounded answer is ~€0. Conclusion robust to the soft outbound numbers (the inbound term, n=11,739, collapses it alone).

**The one open lever (logistics, not modelling):** the €955 inbound sprinter. If Güll **shares it with other customers**, Picanova's per-parcel share drops below the €2.30 a Picanova-only 52/pallet implies, and Güll could return to contention. Question for the logistics manager (also: space-cap vs weight-cap binding). Until then Güll is parked on cost.

**Sibling flag:** Austrian Post carries the identical unvalidated `LINE_HAUL_PARCELS_PER_PALLET = 150.0` (Güll's 150 was *borrowed from AP*). Same mart finding applies; not corrected (out of scope). [[feedback_fix_the_class_across_sibling_consumers]]. The whole episode is a clean [[feedback_revalidate_borrowed_constants]] instance: a borrowed constant sitting on a fixed-cost divisor has leveraged impact — a €163k headline that was ~300× the grounded €520.
