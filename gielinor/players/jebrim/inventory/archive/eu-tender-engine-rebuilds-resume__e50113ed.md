# Resume — EU Tender engine rebuilds (S104)

**Status:** in-progress · session e50113ed · 2026-05-27
**Quest:** `quest-log/in-progress/S104_e50113ed_eu-tender-engine-rebuilds.md`
**Continues:** S103 (d4f287de, clean checkpoint). **Repo:** out-of-tree `bi-analytics-main/NFE/projects/2_EU_tender_2026/2_analysis/carriers/`

## Where we are
- **maersk-3.0.0** — DONE + COMMITTED (tender `3b86d6a`, S102). Reference pattern (two-index fuel by region, oversize modules).
- **hermes-2.0.0** — DONE + VERIFIED + COMMITTED (tender `990d61c`, S103). Reference pattern (per-month base-2021 fuel + per-country surcharge `replace_strict`).
- **dhl_express** — `dhl_express-1.1.0`, NOT started. **← active build this session.**
- **austrian_post** — NOT started (next).
...
4. Commit pathspec-scoped (`git commit -- <explicit files>` — tree has heavy unrelated WIP; never `git add .`), local-only, "EU tender: <carrier>-X.Y.Z rebuild (S104)" + Co-Authored-By trailer.

## Files to read first
- `carriers/dhl_express/` (constants + surcharges + calculate + tests)
- `carriers/maersk/` (committed 3.0.0 pattern) + `carriers/hermes/` (committed 2.0.0 — per-month fuel + per-country surcharge `replace_strict`)
- `carrier_responses_to_open_questions/DHL_express/REVIEW_CONCLUSIONS.md`
...
- Brain-side S099/S102/S103 + this S104 records uncommitted — awaiting principal go.
