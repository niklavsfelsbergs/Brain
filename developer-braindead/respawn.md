...
**Last updated.** 2026-05-22 (end of [[S038]] — closed the promote/consult loop diagnosed from the principal's "we are underutilizing a lot of things" frame. Two structural fixes shipped: respawn reads every `.md` in `confirmed/` (not just `current.md`) so promoted knowledge actually loads; 12 invisible Jebrim confirmed entries now load on next respawn. Habit-shaping aids: `/drafts` slash command + `drafts-triage` ritual; close-session step 4 agent-initiative scan for stale-done quests; step 8 catch for orphan untracked quest-log files; Guthix routing heuristic in `meta/communication-protocol.md`. [[D-023]] captures full design. Prior carries: [[S033]] visualizer audit + 12 fixes; [[S034]] Guthix consultation mode; [[S037]] switchboard Phase 3 click-to-focus.).
