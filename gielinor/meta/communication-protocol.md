...
- **Active actor by mode.** Player session → `<player>.txt` (e.g., `jebrim.txt`, `zezima.txt`). Dev-brain session → `braindead.txt`. Unscoped session → `wisp.txt`. Bankstanding ritual (any session) → `guthix.txt` (the deity who tends the brain; see [[guthix]] and `modes.md` bankstanding section).
