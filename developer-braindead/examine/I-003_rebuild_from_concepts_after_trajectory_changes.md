...
**Context.** The switchboard line ran ~30 sessions ([[S037_terminal_switchboard_phase_3_click_to_focus|S037]]–[[S063_switchboard_feed_liveness_and_cache_fix|S063]]) of iterative patching across **repeated trajectory changes**: an isometric map with sprites / walk animations / wander → [[D-026_switchboard_promotion]] killed the map and promoted the two load-bearing panels to `switchboard/` → [[D-028_switchboard_cockpit_rebuild]] / [[S064_78824901_switchboard-cockpit-rebuild]] rebuilt the whole thing greenfield as a standalone fleet cockpit in `cockpit/`. The tell that the patching had gone bad showed up repeatedly:

- Four-plus sessions ([[S047_a110d573_visualizer-cluster-stack-and-helmet-clearance|S047]]–[[S051_8bf3fdb7_visualizer_tree_and_bubble_scale|S051]]) all touched the map; most were *debugging* it, not improving it.
- The "each fix looks like the last bug" pattern ([[S048_b070e9be_visualizer_manifest_driven_sprite_sync|S048]]) — derived state maintained by an event stream had structurally diverged from the sidecar's truth; three patch rounds each narrowed the symptom and the next bug wore the same face. Only an architectural inversion fixed it.
- [[S063_switchboard_feed_liveness_and_cache_fix|S063]]: the principal hit *"I need to test it from a clean slate"* and found the stale-JS cache disease that had silently taxed nearly every switchboard session.
