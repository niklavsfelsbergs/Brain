...
Source: `bi-analytics/NFE/dashboards/shipping_costs_monitoring_nextjs/pipeline.py`. Harvested [[S097_1ce9fc1f_scm-daily-product-temp-spill-oom|S097]] (2026-05-26); ties to [[S069_006248ef_pipeline-oom-hardening]].
...
- **`_write_daily_product_summary` ~L2444** — UNNEST(`shop_order_groups`) + 9-dim GROUP BY. `memory_limit='8GB'` (was 2GB until [[S097_1ce9fc1f_scm-daily-product-temp-spill-oom|S097]]).
...
1. **RAM OOM** — unconfigured (or too-high) `memory_limit`; the connection grabs most of the pod's RAM and the allocation fails. [[S069_006248ef_pipeline-oom-hardening|S069]]'s fix: cap `memory_limit` + give a `temp_directory` so it spills.
2. **Temp-spill OOM** — `_duckdb.OutOfMemoryException: ... max_temp_directory_size`. The `memory_limit` is so low it forces heavy spill, but `temp_directory` rides the **DATA volume** which is near-full (raw.parquet + processed monthlies + intermediates), so `max_temp_directory_size` (defaults to free disk) is tiny. [[S097_1ce9fc1f_scm-daily-product-temp-spill-oom|S097]]'s case: 2GB cap → spill > 2.3 GiB free → OOM on disk.
...
**Volume context:** mart roughly doubled ([[S068_363fdec7_v1-freeze-mart-knowledge-reconciliation|S068]] floor 2023-01-01); full fact ~18M rows, ~13M shipments/refresh. The pipeline was originally sized for ~8M — both connections are volume-sensitive and may need re-tuning as it grows.
