...
- Entered dev-brain mid-conversation via "lets develop gielinor". Principal: a leftover dev that pings Windows when a claude session finishes is now redundant with the cockpit's notifications — find it and remove it. Posted [[S091_remove_leftover_windows_toast_notifier]] `OPEN`; one live sibling (braindead-327b1216, cockpit chat-UI **discussion-only**) — no file overlap.
