...
- **Parallel penguins.** First cut treats each penguin as singular like dwarves/gnomes; parallel-instance routing ([[D-017_parallel_player_instances|D-017]] / [[D-019_parallel_braindead_and_comms_channel|D-019]] style) deferred until use shows it matters. Penguins are sub-agents (always unique IDs P1, P2, …), so the parallel-instance concept doesn't apply the same way — but a single principal could spawn 3 penguins simultaneously for cluster-research. The gather-slot scaffolding at the iceberg STAND handles 3+ co-located penguins.
