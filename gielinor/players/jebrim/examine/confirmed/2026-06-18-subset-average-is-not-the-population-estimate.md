...
## The lesson
A figure computed over a **selected subset** carries that subset's selection bias; it does not describe the population. The un-invoiced remainder is selected *by what hasn't billed yet*, which correlates with cost — so its average is structurally unlike the whole. Before generalizing a sub-population statistic to a headline ("the estimate is fine"), check what selected that sub-population and whether the selection correlates with the metric. The clean test was the **same-parcel** comparison (invoiced parcels, expected vs real on identical rows) — zero selection confound. I should have reached for that first instead of relaying a tail-average as a class claim.
