...
- **`quest-log/`** — One file per Guthix session that produces a trace. Two prefixes: **`B-NNN_*`** for bankstanding passes (the counter is Guthix's own ritual history, distinct from any player's S-NNN); **`G-NNN_*`** for consultation sessions whose conversation produced something worth surfacing on next respawn (load-bearing reflection, deferred decision, flagged drift). Most consultations leave no entry. Both prefixes count separately — the next bankstanding stays `B-NNN`, the next consultation-with-trace is `G-NNN`. The entry records what he covered, what he proposed (bankstanding), or what was discussed and concluded (consultation).
- **`inventory/`** — Resume state for the *current* in-residence Guthix session. For bankstanding: "Phase 0 done for Jebrim, Zezima next." "Drafts triage covered examine/, niksis8/ remaining." For consultation: rarely used, but available if a long thinking session needs to carry state across turns. When the session closes cleanly, this clears or moves to the matching quest-log entry. When work spans sessions, this is what next-session-Guthix reads first.
