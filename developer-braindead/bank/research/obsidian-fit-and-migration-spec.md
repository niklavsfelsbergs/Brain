...
- **NEXT:** (1) **verify in Obsidian** — open `developer-braindead/` as a vault; the decision + session graphs + backlinks should light up (principal eyeball; not headless-checkable). (2) extend to **gielinor/** (reuse the script, `--vault gielinor/`). (3) two fuzzy D-001 calls (`plan.md:29`, `S003:13`) left as-set — both resolve. (4) cross-brain ~4% refs decision. Roadmap: [[plan]] §O.
