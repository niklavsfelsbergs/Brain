...
- No writes to any `confirmed/` path. Applies across all scopes — global `examine/`, `niksis8/`, `lorebook/`, per-player `examine/`, `niksis8_character/`. **Two bypasses:** `braindead` always (the construction crew); and Guthix during **bankstanding/alching** with an explicit-grant floor-unlock marker ([[D-036_guthix-floor-unlock-in-bankstanding|D-036]]).
- No file deletes. The agent moves files into the corresponding `archive/`. **One bypass:** `braindead` only — the floor-unlock does *not* extend to deletes.
