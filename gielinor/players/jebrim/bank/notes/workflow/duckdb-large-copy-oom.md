...
**As of:** 2026-05-25 ([[S069_006248ef_pipeline-oom-hardening|S069]]). **Source:** `Documents/GitHub/bi-analytics/NFE/dashboards/shipping_costs_monitoring_nextjs/pipeline.py`.
...
**The trap that bit us:** the connection had a comment claiming "DuckDB hash-joins on parquet files with automatic disk spilling and is rock-solid for this size" — but it set *none* of the above, while a sibling connection in the same file set `memory_limit` + `temp_directory`. A comment asserting a property is not the property. It survived until the source mart ~doubled ([[2026-05-23-disk-absence-needs-non-gitignore-aware-listing]] era / [[S068_363fdec7_v1-freeze-mart-knowledge-reconciliation|S068]] reload: floor really 2023, ~18M-row fact) and crossed the threshold.
