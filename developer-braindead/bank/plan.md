...
- §P.3 `[ ]` **NEXT — item 5: golden-file verification harness** so every later catalogue item ships with a `check.py`. [[S115_a9fe4749_khaan-failure-receipts-and-hygiene|S115]] already did one such check ad hoc (the `.py` BANNER == registry born-link block byte-match) — item 5 formalizes that into a `verification/` folder + `check.py` per ritual (respawn load-order, drafts-triage verdict table, born-link wrap/block decisions, the four banner byte-matches, each write-boundary hook vs synthetic payloads), runnable alongside `py_compile`/`node --check` at pre-commit. Steps 3+ of the sequence (positive-gate-bundle, 5-lens doctrine, scored recall/digest/charges) follow; autonomous items (A/B/C…) are the §C-phase, much-later. See the catalogue HTML for the full ordering.
