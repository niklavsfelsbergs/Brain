...
- `docs/ASSUMPTIONS.md` Maersk blocks (lines 13-25 strict-add, 134-199 post-reply table + S120 peak/ROW-demand entries).
- `docs/technical/README.md` (template).

## Engine snapshot
- Version `maersk-3.0.0` (S099/S120 cascade, PLAN §B.19). Two services HD-only: eu_hd (gross, 26 EU+adjacent, LI→CH), row_hd (FedEx Economy, max(gross,LWH/5000), ~150 ROW via zone map).
...
Engine constants reconcile cleanly to REVIEW_CONCLUSIONS + ASSUMPTIONS (fuel, tolls, oversize scalars, ROW 4th trigger, DE routing=0, peak/ROW-demand deferrals all match what S099/S120 specified). Discrepancies are doc drift + two substantive opens:
