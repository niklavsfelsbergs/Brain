...
**Anchor:** continues this-day workbench (May 2026 TCG shipping-quota outliers, where I produced the €61.7K UPS over-max cluster figure) + the S253 (5974b4ee) shipping-agent run-yourself policy.
...
Process miss flagged by principal: I **reflex-spawned the shipping-agent** for this — a one-`GROUP BY` reconciliation — when the S253 default is run-it-myself. Triggered by the spawn-list naming "charge-bucket decomposition" as a topic.
...
The original "something's wrong" = three lens/scope mismatches stacked: total parcel cost vs one surcharge line; order-month vs invoice/transaction date; and "over max" being one of several stacked UPS surcharges (LPS/SOV/OVR/OML/AHC — codes per S243). No mis-billing; the apparent "2 lines per parcel" on dimensional surcharges = real €499 line + harmless €0.00 companion. Signal that fell out: May over-max parcels cost €1,313 each vs April's €708 — the SOV peak demand surcharge hits May, not April.
...
Not touched: `cue_registry.py` + `bank/domains/shipping-mart.md` (the other 2 S253 homes) — they don't carry the methodology-bullet prose; nothing to re-scope there.
