...
- **Migrate S014's existing resume sections in this session.** Considered. Held back — the migration note in close-session.md step 3 says "one-time per quest" on next close-session pass; S014's migration happens organically when S014 next closes a session. Doing it ad-hoc here would land outside any ritual and bypass the new soft-block check anyway.
...
- **S014's migration of resume sections.** Happens on next close-session pass per the migration note. One-time.
...
- **Possible `I-NNN` observation about the quest-log-as-vacuum pattern.** S018 surfaced the pattern but the I-NNN log entry wasn't drafted in this session. Candidate for next bankstanding.
